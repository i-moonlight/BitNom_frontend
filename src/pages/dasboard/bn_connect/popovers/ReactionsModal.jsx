      ...
      <Grid container>
        ...
        <Grid item lg={6} md={8} sm={10} xs={10}>
          <Card>
            ...
            <Divider />
            <CardContent>
              {value === 0 && resourceReactions?.reacted_to_by?.length > 0 && (
                <ReactionList reactions={resourceReactions?.reacted_to_by} />
              )}
              {value === 1 && resourceReactions?.reactions?.likes > 0 && (
                <ReactionList reactions={likes} />
              )}
              {value === 2 && resourceReactions?.reactions?.loves > 0 && (
                <ReactionList reactions={loves} />
              )}
              {value === 3 && resourceReactions?.reactions?.dislikes > 0 && (
                <ReactionList reactions={dislikes} />
              )}
              {value === 4 &&
                resourceReactions?.reactions?.celebrations > 0 && (
                  <ReactionList reactions={celebrations} />
                )}
              {(value == 0 && resourceReactions?.reacted_to_by?.length < 1) ||
              (value == 1 && resourceReactions?.reactions?.likes < 1) ||
              (value == 2 && resourceReactions?.reactions?.loves < 1) ||
              (value == 3 && resourceReactions?.reactions?.dislikes < 1) ||
              (value == 4 && resourceReactions?.reactions?.celebrations < 1) ? (
                <Grid align="center">
        ...
        <ListItem key={reaction?.user_id?._id} button>
          <ListItemAvatar>
            ...
            >
              {reaction?.user_id?.profile_pic
                ? ""
                : getUserInitials(reaction?.user_id?.displayName)}
            </Avatar>
          </ListItemAvatar>
          <ListItemText
