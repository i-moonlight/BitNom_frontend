...
export default function MuiCarousel() {
  ...
  return (
    <div className='h-100 py-5'>
      <Typography color='primary'> STH: {JSON.stringify(query)}</Typography>
      <Container>
        <div className='container'>
          ...
          >
            {splitted?.map((split, indexUpper) => (
              <div key={split[0]} className='row text-white bg-theme my-5'>
                {split?.map((item, index) => (
                  <div key={item} className='col-xs-12 col-sm-6 col-md-4'>
                    One of three columns {index + 1} of {indexUpper + 1} [
